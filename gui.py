...
class MainWindow(QMainWindow):
    ...
    def evt_shutt(self):
        ...
        fig = px.imshow(self.a, text_auto=False, labels=dict(color="Temperaute, Celsius"), template = 'plotly_dark', title='SF {0:g}, Minimum: {1:0.2f} Celsius, Maximum: {2:0.2f} Celsius, Avg.: {3:0.2f} Celsius, Thermistor Value: {4:0.2f}'.format(self.sFrame,minHeat,maxHeat,avgHeat,float(self.f)))
    ...
    def evt_btn_request(self):
        ...
        if enter:
            ...
            for i in range(self.q):
                ...
                minHeat = np.min(self.a)
                avgHeat = np.average(self.a)
                fig = px.imshow(self.a, text_auto=False, labels=dict(color="Temperaute, Celsius"), template = 'plotly_dark', title='D{0:g} F {1:g}, Minimum: {2:0.2f} Celsius, Maximum: {3:0.2f} Celsius, Avg.: {4:0.2f} Celsius.'.format(self.dataSet,self.frame,minHeat,maxHeat,avgHeat))
